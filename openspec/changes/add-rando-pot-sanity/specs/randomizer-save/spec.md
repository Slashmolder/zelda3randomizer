...
- **WHEN** a `pot_shuffle = All` slot with ~1163 placed locations is written and read back by a pot-capable binary
...
- **WHEN** a pre-pot-shuffle (512-era) binary loads a sidecar whose slot has a `placement_table_size` implying ~1163 locations

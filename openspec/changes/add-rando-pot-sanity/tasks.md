...
- [ ] 1.1 **Typed audit of ALL location-id-keyed arrays/constants/guards** (NOT a
...
- [ ] 1.2 Add a `_Static_assert` per raised site tying it to `LOC__COUNT` (≥), so a
  future registry past capacity is a build break, not a silent overflow/drop/truncate.
- [ ] 1.3 `make clean` + WSL `make -j zelda3` (`-Werror`) + `--rando-selftest` green;
  **corpus byte-identical** (no registry change, no version bump yet). Checkpoint.

## 2. Pot enumeration generator — D2, D8, D11

- [ ] 2.1 Write `assets/scripts/gen_pot_tables.py`: parse `zelda3_assets.dat` (asset 3
...
- [ ] 2.2 Emit append-only registry rows + `pot_table.gen.bin` → `src/rando/pot_lookup.h`
  (**sorted** `(room,pos)→LOC` for binary search). Wire into the build like
  `chest_table.gen.bin`.
- [ ] 2.3 **Pot location type migration (D11)** — update ALL: schema enum
...
- [ ] 2.4 Logic entries: bind each pot to its room's region, `can_reach: TRUE()`. For
...
- [ ] 3.1 Add `pot_shuffle` enum to `RandoSettings`; pack canonical byte `[27]` bits
...
- [ ] 3.2 **Active filter (D1/D9):** skip out-of-scope pot locations in the
...
- [ ] 3.3 **`ITEM_Nothing` (D6):** add to `item_registry.yaml`; fill empty-pot LOCs in
...
- [ ] 3.4 **`Placement_Lookup` binary search (D10)** + sortedness invariant at EVERY
...
- [ ] 3.5 **Key economy (D7):** pot-key locations follow `dungeon_small_keys_mode` —
...
- [ ] 3.6 Bump `kGeneratorVersion`. `make clean` + build + `--rando-selftest`.
...
- [ ] 4.1 `Dungeon_GetPotLocation(room, pos4) → LOC` (binary search over `pot_lookup.h`).
- [ ] 4.2 Hook at the **TOP of `RevealPotItem`** (before the secret scan; covers all
...
- [ ] 4.3 **Checked-pot behavior:** item-pots re-drop vanilla content; **key-pots (and
...
- [ ] 4.4 Recolor in `RoomDraw_SinglePot`: **mask out palette bits 10-12 of the four
...
- [ ] 5.1 `pot_shuffle` four-value selector in the native settings window + 1-2 line
  tooltip; wire through the slot generator. Surface the door-shuffle key-pot
  restriction (D7).
- [ ] 5.2 **SNES HUD location tracker (`hud.c:1863`)**: hide/page/summarize pots (it
  loops over locations and breaks with 800+ rows) — NOT just the native window.
- [ ] 5.3 Native/auto tracker + reach panel: group pots by room / "show pots" toggle;
...
- [ ] 6.1 `--rando-selftest`: `(room,pos)→LOC` round-trip, tier nesting, off-graph
  byte-identical, `ITEM_Nothing` logic no-op, `Pot`-type codegen round-trip,
  placement-table sortedness.
- [ ] 6.2 Fresh-eyes audit (independent reviewer) over the committed diff, briefed on
...
- [ ] 6.4 Reconcile spec deltas + design vs as-built; update `docs/randomizer.md` and a

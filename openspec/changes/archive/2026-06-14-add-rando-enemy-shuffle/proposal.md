...
**MVP implemented + squash-merged to main (2026-06-08, commit `807cce8`), then widened in-place.** Sprite-type substitution is live as a default-off rando slot axis: WSL `-Werror` build green, `--rando-selftest` (incl. `EnemyShuffle_SelfCheck`) green, corpus byte-identical (0/112), runtime-confirmed substituting. The variety-unlock machinery has since landed under the same axis: subgroup slots `0..3`, generated Enemizer-sourced tables, dungeon-overlord spawned-slot decode, and per-seed HP/contact-damage randomization (bosses exempt). Runtime sheet widening is currently palette-gated off after runtime validation; enemy type substitution and stat randomization remain live. The caught render regressions have been validated after the stale-sheet, context, and overworld-palette gates; water-only room classification, palette-aware sheet widening, and OAM-footprint modelling are not yet guaranteed. Authored 2026-06-07; revised 2026-06-08 and reconciled again 2026-06-13 after the v3 widening follow-ups. See [design.md](design.md) for the algorithm + hazards; [README.md](README.md) for the file index.

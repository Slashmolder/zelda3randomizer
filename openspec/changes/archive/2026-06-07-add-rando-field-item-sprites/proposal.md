...
- **Risk:** the load-bearing risk is graphics/VRAM (per-screen slot + palette correctness); mitigated by the spike-first task order and an end-to-end playtest (the playable-slot path has no automated test). Upstream runtime reference is the ALTTPR asm (`C:\src\z3randomizer` — `decompresseditemgraphics.asm`, `itemdatatables.asm`, `heartpieces.asm`), not the PHP (placement-only).

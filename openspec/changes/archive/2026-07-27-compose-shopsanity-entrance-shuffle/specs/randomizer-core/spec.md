...
NOTE: this scenario's TITLE is obsolete and is retained only because the
archiver cannot express a scenario rename inside a MODIFIED requirement — it
reads a missing title as an accidental drop and refuses. The canonical spec's
copy is renamed to "Shopsanity survives canonical serialization under
cave-entrance shuffle" in the same change; the assertion below is authoritative.

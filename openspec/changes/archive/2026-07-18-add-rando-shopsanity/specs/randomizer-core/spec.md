# randomizer-core — delta for add-rando-shopsanity

> **Sequencing note (RECONCILED 2026-07-16).** Authored against the
> post-`add-rando-key-rings-skeleton-key` baseline and verified against
> as-merged `main` (`a82667d9`): `kSettingsCanonicalLen` 31; byte [30] =
> `key_rings` bits 0-1 (`off=0`, `random=1`, `all=2`), `skeleton_key` bit 2
> (`kSkeletonKeyAxis_Enabled`), bits 3-7 refused via
> `kKeyRingsAxis_DefinedMask`. The keyrings archive documented byte [30] in
> its own ADDED requirement rather than editing this table, so this
> restatement also completes the canonical table with the as-built [30] row.

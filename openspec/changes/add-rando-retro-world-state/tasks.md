...
- [x] 1.1 Enumerate the 42 shop entities in `../alttp_vt_randomizer/app/Region/Standard/**/*.php`. Run `grep -hE "new Shop|new Shop\\\\(Upgrade|TakeAny)"` and record per-shop file/line. Classify each as `Shop` / `Shop\Upgrade` / `Shop\TakeAny`. (Slice 3a only enumerates the 9 regular + 1 Upgrade; the 22 TakeAny lines move to 3b.) <!-- done: audit.md §"Retro shop provenance" §1.1. Census: 9 Shop + 1 Shop\Upgrade + 31 Shop\TakeAny = 41 entities, each with file:line + door_id. --> <!-- CORRECTION: TakeAny count is 31, NOT 22 (spec estimate stale); total is 41, NOT 42. 3b must size for 31. Full TakeAny census recorded in audit.md. -->
- [x] 1.2 For each `Shop` and `Shop\TakeAny`, enumerate the purchasable inventory by inspecting `->setInventory(...)` calls. Record per-slot item id and rupee cost. Expected total: ~30-40 purchasable slots. <!-- done: audit.md §1.2. Inventory is via ->addInventory(slot, Item::get(name), price[, max]); 9 regular shops × 3 + Capacity Upgrade × 2 = 29 slots (within ~30-40 est.; TakeAny adds more in 3b). Per-slot item+price+file:line tabulated, with ALTTPR→fork item-name alias map. -->
- [x] 1.3 Add a new section `audit.md §"Retro shop provenance"` to the parent change's audit (or as a Phase B addendum) listing every shop with PHP source-line citation per `add-randomizer-support / audit.md §0.10` translation discipline. <!-- done: created openspec/changes/add-rando-retro-world-state/audit.md §"Retro shop provenance". -->
- [x] 1.4 Cross-check: confirm no shop slot collides with an existing `LOC_<id>` in `assets/rando/location_registry.yaml`. Each new shop slot gets a unique numeric id appended to the registry (append-only convention preserves Open/Standard digests). <!-- done: audit.md §1.4. NO collisions — the 29 Slice-3a slots already exist as appended ids 237-265 (location_registry.yaml:374-443), all world_state_filter:[retro]. Independently re-derived from PHP and cross-validated: item mapping + source line + slot order all match exactly. -->

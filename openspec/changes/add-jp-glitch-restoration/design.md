...
- **Superspeed / Spindash / Mirror Block Erase: NEEDS-MORE-WORK.** No JP↔US code delta in the anchored routines (state-leak / input-race; no US guard to bypass). Not shipped. Next step: diff the controller-read/NMI input-latch + area/stair transition handlers and confirm against a JP emulator capture before any attempt — likely not reproducible in the fixed C control flow.

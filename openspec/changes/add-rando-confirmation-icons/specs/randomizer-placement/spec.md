...
When the dispatcher returns `kRandoLttpSkip` (direct-grant items: `HalfMagic`, `QuarterMagic`, `TriforcePiece`, prize bits, dungeon-item bits), the call site SHALL invoke `Rando_ShowDirectGrantConfirmation(item_id)` — extending the Phase A signature (`void`) to take the granted item id. The implementation SHALL spawn a per-item-type icon ancilla that draws above Link in the same screen-position style as the vanilla item-receipt ancilla. Audio + HUD refresh behavior from Phase A §7.6 SHALL be preserved; the visible ancilla is added alongside, not replacing.
...
#### Scenario: Triforce piece grant pops a Triforce icon
- **WHEN** the player picks up a TriforcePiece from a §6.x direct-grant site (e.g., a Pyramid Fairy item drop placed there by the shuffler)
- **THEN** the standard item-receipt sound plays, the HUD refreshes, and an item-receipt ancilla draws a Triforce-piece icon above Link's head matching the Triforce icon used on the HUD
...
#### Scenario: Item not in the icon table falls back to audio-only
- **WHEN** a direct-grant call site passes an item id that has no entry in `kDirectGrantIcons`
- **THEN** the helper plays the audio + HUD-refresh portion only (Phase A behavior) and does NOT spawn a draw-tile ancilla; a debug-build assert flags the missing entry so the codegen table can be filled in

...
- [x] 7.3 Pyramid Fairy site. *(SUPERSEDED by add-rando-fairy-chest-model: the Pyramid Fairy no longer uses a bottle/trade — it grants Left/Right on contact (chest model). The §7.6 confirmation cue still fires from that path via `Rando_ShowDirectGrantConfirmation(Rando_LastDispatchedItemId())`. Owner VERIFIED 2026-06-03 the pond grants items on contact; the icon contract is unchanged.)*

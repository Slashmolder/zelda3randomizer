...
- [ ] 6.3 Manual verification: generate a seed before and after this change with the same `--seed` and confirm `placement_digest_hex` is byte-identical (`docs/randomizer_playthrough.md` cross-platform digest check). *(Deferred to corpus comparator run — code review shows no placement-affecting changes: only call sites added, no logic/macro/op changes.)*

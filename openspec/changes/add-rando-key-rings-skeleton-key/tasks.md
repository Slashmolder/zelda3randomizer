...
- [ ] 1.1 Append `KeyRing_*` IDs 220..232 and `SkeletonKey` ID 233 to
  `assets/rando/item_registry.yaml`; regenerate item IDs, names, dispatch tables,
  and assert `ITEM__COUNT <= 256`.
- [ ] 1.2 Add explicit small-key↔ring↔rando-dungeon↔game-key-slot mapping helpers
  and selfchecks, including Hyrule Castle proper's Escape/Sewers key-slot fold.
- [ ] 1.3 Generate/centralize the complete authored key-stock count for all 13
...
- [ ] 1.4 Add one shared Key Ring icon and one Skeleton Key icon to the custom item
  gfx generator/blob, palette path, direct-grant icons, field-item sprites, and
  item-name presentation.
- [ ] 1.5 Add codegen/build wiring and append-only registry/art freshness guards.

## 2. Settings, selection, provenance, and compatibility

- [ ] 2.1 Add `key_rings=off|random|all` and `skeleton_key=false|true` to
  `RandoSettings`, defaults, validation, CSV parsing, presets, and fixed-settings
  fixtures.
- [ ] 2.2 Append canonical byte `[30]` (ring bits 0-1, Skeleton bit 2, bits 3-7
  refused), grow `kSettingsCanonicalLen` 30→31, and add round-trip/undefined-bit
  selfchecks.
- [ ] 2.3 Implement `Settings_EffectiveKeyRings`: normalize Off for effective
  Vanilla small keys and Retro generic keys without overwriting the persisted
  requested mode; keep Skeleton Key independent.
- [ ] 2.4 Implement the centralized pre-collapse eligible-family counter over base,
  pot, enemy, and future registered shuffled key sources.
- [ ] 2.5 Implement salted `KeyRings_Select(settings, seed)` without consuming the
...
- [ ] 2.6 Bump `kGeneratorVersion` and corpus `generator_version` together; run the
  generator-version and corpus-version sync guards.

## 3. Item pool, assumed fill, and customizer

- [ ] 3.1 Refactor `BuildItemPool`/callers so seed-derived ring selection is
  available after all active small-key source contributions and before junk pad.
- [ ] 3.2 For each selected family, replace one ordinary small key with one ring,
  remove all other shuffled copies, and let junk pad fill the released slots.
- [ ] 3.3 Classify rings as dungeon progression in the restrictive first placement
  tier; preserve Dungeon confinement and Wild placement behavior.
- [ ] 3.4 Add exactly one non-progression Skeleton Key before junk padding when
  enabled, preserving total placement cardinality.
- [ ] 3.5 Make `OP_ITEM_IS(SmallKey_D)` and `OP_ITEM_IS(KeyRing_D)` candidate-
  equivalent for selected D so forced/forbidden/always-allow key rules remain
  sound.
- [ ] 3.6 Reject customizer unselected rings, duplicate rings/Skeleton Keys, and
...
- [ ] 3.7 Add pool selfchecks for every family and base/pot/enemy/combined source
  matrix; prove one ring + zero shuffled family keys for every selected family.

## 4. Logic and door-shuffle oracle

- [ ] 4.1 Add a centralized effective item-count helper: a held family ring
  saturates that family's small-key count; Retro GenericKey remains mutually
  exclusive and unchanged.
- [ ] 4.2 Route `HAS_ITEM`, `HAS_AMOUNT`, `HAS_ANY_OF`, and `HAS_ANY_COUNT` through
  the helper, with focused predicate selfchecks.
- [ ] 4.3 Route door-shuffle `held_keys[]`, oracle fingerprints/caches, final sphere
  walk, goal verification, and live reachability through the same ring-aware model.
- [ ] 4.4 Materialize the derived owned-ring mask into live `RandoCounts` ring-item
  entries; prove numeric key counters alone never synthesize ring ownership.
- [ ] 4.5 Audit all direct `SmallKey_*` count reads so none bypass the helper.
- [ ] 4.6 Keep Skeleton Key non-progression and absent from every logic/oracle path;
  add an invariant that removing it from counts never changes reachability or goal
  completion.
- [ ] 4.7 Validate ring placement in Standard HCE, Inverted dungeon logic,
  door-shuffle layouts, and dungeon chains without self-locks.

## 5. Runtime grants and key-door behavior

- [ ] 5.1 Add Key Ring direct grant: max-write the target saved key slot with the
...
- [ ] 5.2 Add Skeleton Key direct grant and derived ownership state with the same
  direct-grant confirmation contract.
- [ ] 5.3 At the sole small-key-door payment site, bypass the regular/GenericKey
  decrement when Skeleton Key is owned; preserve the already-open and door mirror
  behavior.
- [ ] 5.4 Prove the big-key-door branch is unchanged and Skeleton Key never sets or
  satisfies a Big Key bit.
- [ ] 5.5 Add runtime selfchecks for in/out-of-dungeon ring grants, HC folding,
...
- [ ] 6.1 Build the 13-bit ring-owned and Skeleton-owned hot cache from installed
  placements plus checked bitmap; update on grant and clear on deactivation.
- [ ] 6.2 Rebuild derived ownership after sidecar activation, bitmap load, snapshot
  cold replay, placement reinstall, race reveal, and customizer slot activation.
- [ ] 6.3 Bump sidecar format 8→9 for the 31-byte settings blob; read v1..v8 with
  versioned shorter layouts and zero-extension, and document pre-v9 refusal of v9.
- [ ] 6.4 Update snapshot RandoState settings parsing, suppressed-spoiler layout and
  size, native settings persistence, and every `kSettingsCanonicalLen`-coupled
  assertion/buffer.
- [ ] 6.5 Update v2 share-string capacity and exact-length fixtures (31-byte
  ...
  zero-extension.
- [ ] 6.6 Add save/load and snapshot round trips for unowned/owned rings, owned
  Skeleton Key, spent counters, and slot deactivation/reactivation.

## 7. UI, spoiler, tracker, autotracker, and docs

- [ ] 7.1 Add native-window `Key rings` selector and `Skeleton Key` checkbox near
  dungeon small-key settings, including effective-Off explanation and “bonus only”
  copy.
- [ ] 7.2 Update settings summaries, CLI help, file-select/native persistence, and
  `docs/randomizer.md` with interactions and non-goals.
- [ ] 7.3 Emit spoiler JSON/text fields for requested/effective mode, eligible and
  selected masks/names, selection version, and Skeleton enabled state.
- [ ] 7.4 Add ring and Skeleton ownership to item tracker/map/reach panels as
  appropriate; keep the numeric remaining-key HUD/counters intact.
- [ ] 7.5 Extend autotracker output with selected/owned ring families and Skeleton
  ownership using an additive schema field.
- [ ] 7.6 Make rings progression-hintable and Skeleton bonus-hintable without
  letting Skeleton enter progression-only accessibility/hint selection.

## 8. Automated validation

- [ ] 8.1 Run `openspec validate add-rando-key-rings-skeleton-key --strict` and
  `openspec validate --all --strict`.
- [ ] 8.2 Run `git diff --check`, codegen strict/freshness/wiring checks, registry
  append-only checks, audit guard, settings/share/save selfchecks, and corpus schema
  validation.
- [ ] 8.3 Build Release x64 and run `--rando-selftest` plus `--door-selftest`.
- [ ] 8.4 Prove feature-Off placement tables are byte-identical to the pre-change
  generator for exact seeds; rebaseline only the intentional provenance/hash
  changes.
- [ ] 8.5 Run corpus rows for Off, Random Dungeon, Random Wild, All, pot keys,
...
- [ ] 8.6 Run exact-seed determinism/reload checks: selected mask, placement table,

...
Total: 2 active changes (`add-rando-switch-swkbd` [parked] + `add-rando-enemy-sheet-widening`) + 35 archived. The archived set currently contains 31 randomizer/randomizer-adjacent changes, 2 native-UI changes, and 2 non-rando JP-feature changes. Run `openspec validate --changes` to confirm all validate.
...
- [`add-rando-enemy-sheet-widening`](add-rando-enemy-sheet-widening/) — **rando slot axis — active (in progress).** Follow-on to `add-rando-enemy-shuffle`: enables the all-slot sprite-GFX-sheet reshuffle (dungeon sprite-palette signature + verify-then-commit fillability), a position-aware sheet loaded-check, and a contact-damage class-2 exclusion. Runtime-only; placement/sphere digests stay byte-identical. Awaiting owner playtest before archive.

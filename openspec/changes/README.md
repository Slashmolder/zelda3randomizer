...
| **D** | D2 | [`add-rando-customizer-mode`](add-rando-customizer-mode/) | Manual per-location placement (manifest pins + assumed-fill completes) + `pool_overrides` — headless CLI (`--generate-seed`/`--generate-slot --customizer=`), playable-slot path, native-window toggle + manifest field. Race mode mutually exclusive. `customizer_active` = canonical `[26]` bit1 | Built; in-window playtest pending; share-string `customizer_seed` deferred (§6.4) | **Yes** | built, playtest-pending |
...
- [`add-rando-share-string-v2`](add-rando-share-string-v2/) — **rando UI/format change — implemented 2026-06-12, owner playtest pending (kGen: No).** Share-string **v2** embeds the full 28-byte canonical settings (`ZRS2 | generator_version | settings_len | canonical | seed_u64 | crc16` = 44 bytes → exactly 71 base32 chars) so "Paste share string" restores settings AND seed losslessly; v1 (50-char) strings decode forever (seed-only + warning). v2 is **UI-transport only** — sidecar/ZRSR/spoiler-filename/`meta.share_string`/banner keep the v1 identity form, so NO save/race/corpus impact and **no kGen bump**. Also ships the Generate-time confirmation modal when settings drift from the last pasted string. Customizer seeds are fenced out of v2 until `customizer_seed` share encoding lands; Switch entry stays v1.
- [`add-rando-enemy-shuffle`](add-rando-enemy-shuffle/) — **new rando slot axis.** Sprite-type substitution (which enemy spawns where), GFX-sheet-constrained, deterministic per-seed, mirroring the boss-shuffle install model. Placement byte-identical for all seeds (sweep-confirmed logic-free); `enemy_shuffle` packs into canonical byte `[26]` bit0 so `kSettingsCanonicalLen` stays 28 and the default `settings_hash` is unchanged. The all-slot sheet-group reshuffle + per-seed enemy HP/contact-damage randomization ship under the same axis; bosses are exempt from stat scaling. Killable-thief/bush/absorbables remain deferred (`tasks.md` §7). Render/crash/softlock playtest pending.

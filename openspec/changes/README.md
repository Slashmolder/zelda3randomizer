...
| **B** | 3-i | [`add-rando-race-mode-reveal-ui`](add-rando-race-mode-reveal-ui/) | File-select per-slot "Reveal Spoiler" action menu + settings-screen suppression-warning string (the unbuilt UI SHALLs carved from #3) | Scaffolded (proposal + spec delta + tasks; 0 impl) | No | UI-only |
...
| **C** | C1 | [`add-rando-entrance-shuffle`](archive/2026-06-05-add-rando-entrance-shuffle/) | Multi-mode composable entrance shuffle — Simple/Restricted/Crossed + coupled built & playtested; per-seed two-mechanism reachability (cave region-reassign + dungeon edge overlay). Archive also `REMOVED` the retired `RegionRemap` baseline requirement | ✅ **Archived 2026-06-05** (owner playtest-confirmed; Insanity carved to C1-i; Skull Woods + Link's House are documented partial-coverage deferrals) | **Yes** | ✅ shipped + archived |
| **C** | C1-i | [`add-rando-entrance-shuffle-insanity`](add-rando-entrance-shuffle-insanity/) | Insanity (decoupled) entrance-mode **runtime** — the cave-arrival asset fork (generation + logic shipped with C1; runtime blocked on the fork) | Scaffolded (proposal + spec delta + tasks; 0 impl) | TBD | playtest-gated |

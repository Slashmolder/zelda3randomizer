...
| **D** | D7 | [`add-rando-pot-sanity`](add-rando-pot-sanity/) | Tiered pot checks (`pot_shuffle=keys|contents|all`), generated pot registry/lookup/logic, runtime pot dispatch + glint, 2048-location capacity raise, pot-local guard/corpus workflows. | 🚧 **Active** (40/43; owner playtest closeout remains for Retro/Inverted + dungeon-keys pot seeds) | **Yes** (82→96) | closeout/playtest |
...
Total: 8 active changes (`add-rando-all-enemy-checks` [27/44], `add-rando-dungeon-enemy-checks` [18/25], `add-rando-enemy-drop-sanity` [43/46], `add-rando-enemy-marker-multi-icons` [25/27], `add-rando-pot-sanity` [40/43], `add-rando-seed-qol` [17/26], `add-rando-switch-swkbd` [parked, 0/31], `dungeon-chains` [30/32]) + 37 archived. The archived set currently contains 33 randomizer/randomizer-adjacent changes, 2 native-UI changes, and 2 non-rando JP-feature changes. Run `openspec validate --changes` to confirm all validate.

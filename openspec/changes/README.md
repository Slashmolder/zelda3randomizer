...
| **D** | D10 | [`add-rando-key-rings-skeleton-key`](add-rando-key-rings-skeleton-key/) | OoTR-inspired per-dungeon Key Rings (`off/random/all`) that collapse selected shuffled small-key families to one check, plus a logic-neutral optional Skeleton Key for all small-key doors. | **Spec ready** (0/55) | **Yes** | proposed |

Total: 2 active changes (`add-rando-switch-swkbd`, parked at 0/31; `add-rando-key-rings-skeleton-key`, proposed at 0/55) + 47 archived. Run `openspec validate --changes` to confirm all validate.

...
| **D** | D7 | [`add-rando-pot-sanity`](add-rando-pot-sanity/) | Tiered pot checks (`pot_shuffle=keys|contents|all`), generated pot registry/lookup/logic, runtime pot dispatch + glint, 2048-location capacity raise, pot-local guard/corpus workflows. | 🚧 **Active** (40/42; owner playtest closeout remains for Retro/Inverted + dungeon-keys pot seeds) | **Yes** (82→96) | closeout/playtest |
...
Total: 2 active changes (`add-rando-pot-sanity` [playtest closeout], `add-rando-switch-swkbd` [parked]) + 37 archived. The archived set currently contains 33 randomizer/randomizer-adjacent changes, 2 native-UI changes, and 2 non-rando JP-feature changes. Run `openspec validate --changes` to confirm all validate.

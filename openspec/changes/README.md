...
- [`add-jp-glitch-restoration`](add-jp-glitch-restoration/) — **non-rando game feature.** A single "Restore JP 1.0 glitches" gameplay checkbox (`kFeatures0` bit 18) that re-introduces glitches patched out of the US 1.0 ROM this port reimplements. JP-vs-US disassembly spike done: **3 IMPLEMENTED faithful** (Fake Flippers, Death Hole, Itemdash — exact gated JP↔US deltas, build-verified on `claude/jp-glitch-restoration`, playtest-pending), **3 NEEDS-MORE-WORK** (Superspeed/Spindash/Mirror Block Erase — no code delta). Fake Flippers = gate the single per-frame recheck at `PlayerHandler_04_Swimming` (the entry ejects are byte-identical JP↔US; no 8-frame grace counter exists — the free-swim approximation was rejected on playtest). Default-off + `!ZeldaIsEmulatorAttached()`; no rando/corpus/`kGeneratorVersion` impact.
- [`add-rando-enemy-shuffle`](add-rando-enemy-shuffle/) — **new rando slot axis.** Sprite-type substitution (which enemy spawns where), GFX-sheet-constrained, deterministic per-seed, mirroring the boss-shuffle install model. Placement byte-identical for all seeds (sweep-confirmed logic-free); `enemy_shuffle` packs into a **reserved canonical pad bit** so `kSettingsCanonicalLen` stays 28 and the default `settings_hash` is unchanged (kGen +1 = 60→61 as-built version-locks the live axis). Fresh-eyes-audited (3 HIGH / 5 MED folded in); corpus 0/112 digests changed. MVP is sprite-shuffle-only; HP/damage/etc. deferred. **MVP squash-merged to main 2026-06-08 (`807cce8`); default-off, runtime-confirmed.** Known limitation: low variety — the **sheet-group reshuffle** (design D4) is the deferred variety unlock; playtest pending.

...
| **D** | D7 | [`add-rando-pot-sanity`](archive/2026-07-11-add-rando-pot-sanity/) | Tiered pot checks (`pot_shuffle=keys|contents|all`), generated pot registry/lookup/logic, runtime pot dispatch + glint, 2048-location capacity raise, pot-local guard/corpus workflows. | ✅ **Archived 2026-07-11** (owner playtest-confirmed) | **Yes** (82→96) | ✅ shipped + archived |
| **D** | D7-i | [`add-rando-door-pot-integration`](archive/2026-06-28-add-rando-door-pot-integration/) | Door shuffle × pot shuffle composition: generated pot-door bridge rows, active pot key-source modeling, door-layout pot-tier/digest drift checks, UI/docs/corpus guard updates. | ✅ **Archived 2026-06-28** | **Yes** | ✅ shipped + archived |
| **D** | D8 | [`add-rando-seed-qol`](archive/2026-07-13-add-rando-seed-qol/) | Pause-map remaining checks, instant/fast text, faster fanfares and cutscenes, warp-to-spawn, auto-dash, and discovered entrance/door auto-tracker feed. | ✅ **Archived 2026-07-13** (owner playtest-confirmed) | No | ✅ shipped + archived |
| **D** | D9 | [`add-rando-all-enemy-checks`](archive/2026-07-13-add-rando-all-enemy-checks/) | Honest `enemy_drop_checks=all` tier across static dungeon/overworld enemies, reviewed underworld exceptions, GT minibosses, and repeatable finite scripted encounters, with stable death grants and respawn-safe checked state. | ✅ **Archived 2026-07-13** (owner playtest-confirmed) | **Yes** | ✅ shipped + archived |
| **D** | D10 | [`add-rando-key-rings-skeleton-key`](add-rando-key-rings-skeleton-key/) | OoTR-inspired per-dungeon Key Rings (`off/random/all`) that collapse selected shuffled small-key families to one check, plus a logic-neutral optional Skeleton Key for all small-key doors. | **Ready for owner playtest** (48/55) | **Yes** | implemented |

Total: 2 active changes (`add-rando-switch-swkbd`, parked at 0/31; `add-rando-key-rings-skeleton-key`, implemented at 48/55 and awaiting owner playtest) + 47 archived. Run `openspec validate --changes` to confirm all validate.

...
| **D** | D10 | [`add-rando-key-rings-skeleton-key`](add-rando-key-rings-skeleton-key/) | OoTR-inspired per-dungeon Key Rings (`off/random/all`) that collapse selected shuffled small-key families to one check, plus a logic-neutral optional Skeleton Key for all small-key doors. | **Ready for owner playtest** (50/57) | **Yes** | implemented |

Total: 2 active changes (`add-rando-switch-swkbd`, parked at 0/31; `add-rando-key-rings-skeleton-key`, implemented at 50/57 and awaiting owner playtest) + 47 archived. Run `openspec validate --changes` to confirm all validate.

...
- [x] 1.4 Wire input dispatch — on key press, toggle the in-memory visibility booleans `g_rando_show_item_tracker` and `g_rando_show_location_tracker`. Reset both to `false` on `ZeldaInit` so launch state is always hidden. *(Landed: dispatch at `main.c:1430-1435`; flags defined at `rando.c:408-409` (static-init to `false` satisfies first-launch hidden state); reset to `false` again on slot-deactivate at `rando.c:645-646` inside `Rando_DeactivateSlot`. `ZeldaInitialize` does NOT call `Rando_DeactivateSlot`; the static-init is what guarantees first-launch state. If a future bug requires explicit re-init across slot loads, add `g_rando_show_*_tracker = false;` directly to `ZeldaInitialize` in `zelda_rtl.c`.)*

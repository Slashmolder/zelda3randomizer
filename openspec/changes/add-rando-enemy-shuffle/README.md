...
| [specs/randomizer-shuffles/spec.md](specs/randomizer-shuffles/spec.md) | ADDED — enemy-shuffle behavior, GFX-sheet anti-crash, killable/water beatability invariants |
| [specs/randomizer-core/spec.md](specs/randomizer-core/spec.md) | ADDED — `enemy_shuffle` canonical axis (pad-bit-packed, length stays 28, kGen 58→59, placement byte-identical) |
...
- **Correctness surface**: the per-enemy constraint table (no `SpriteRequirement` analog exists yet — author from Enemizer's `SpriteRequirement.cs` + `Sprite_HEX_*`). GFX-sheet match = anti-crash; `killable && !cannot_have_key` in key rooms + `killable` in shutter rooms + water/directional/room-exclude lists = anti-softlock. Key-room safety keys off the **rando placement table**, not the vanilla key sprite. This table is the **sole** beatability enforcer (logic models no per-room kill-clear) — a bug ships an unbeatable seed the corpus can't catch.
- **Placement byte-identical for ALL seeds** (sweep-confirmed: no non-boss `CanKill` predicate). `enemy_shuffle` packs into a **reserved canonical pad bit** — `kSettingsCanonicalLen` stays **28**, default `settings_hash` byte-identical. kGen 58→59 + corpus regen only advance the manifest version (digests unchanged) and version-lock the live axis.
...
- Builds on **`add-rando-shuffles-and-minigames`** (boss/drop shuffle): reuses the install pattern. Note the struct fields it adds (`boss_shuffle`/`drop_shuffle`, plus `hints` and the entrance pad byte) **already ship in source at kGenVer 58** — but their openspec deltas are unarchived, so the baseline `randomizer-core` normative list and the `randomizer-native-window` parity scenario both lag as-built. This change's deltas carry explicit apply-time reconciliation notes for that drift; enemy shuffle should archive after (or alongside) those changes so the reconciliation happens once.

...
**MVP implemented + squash-merged to main (2026-06-08, commit `807cce8`).** Sprite-type substitution is live as a default-off rando slot axis: WSL `-Werror` build green, `--rando-selftest` (incl. `EnemyShuffle_SelfCheck`) green, corpus byte-identical (0/112), runtime-confirmed substituting (F12 g_ram dump). **KNOWN LIMITATION:** low variety — substitution is constrained to already-loaded GFX sheets + the conservative ~30-enemy table, so swaps are infrequent and same-family. The variety unlock — Enemizer's per-room **sheet-group reshuffle** (`design.md` D4) — is the deferred follow-on; render/crash/softlock playtest also pending. Authored 2026-06-07; revised 2026-06-08 after a fresh-eyes audit (canonical-layout pad-bit packing, overworld substitution mechanism, constraint-flag set, fail-closed install — see `design.md` "Audit notes"). See [design.md](design.md) for the algorithm + hazards; [README.md](README.md) for the file index.

...
| `crystals.tower` | 0..7, `random` | 7 (`random` as above; a rolled 0 pre-opens Ganon's Tower exactly like a fixed 0; since generator version 150 the resolved count also drives the GT entry in placement logic/spheres, so non-7 tower seeds place and hint truthfully) |
...
route remain usable. Entering a Desert Palace auxiliary door does not arm a chain
by itself, but if that route reaches Desert Palace's boss seam, the seam follows
Desert Palace's configured chain successor and uses the matching Desert Palace
auxiliary door as the return origin.
...
compose with normal prize/medallion shuffle, pot shuffle, traps, and enemy/drop
shuffle. When chains are active, small keys and big keys are normalized to
in-dungeon mode so generated logic cannot rely on vanilla in-place key
assumptions after a required dungeon route has changed.
...
DROP index, door region, and source predicate, plus door x ordinary enemy-check
bridge rows carrying door regions, active tier, and source predicates. The door
layout digest includes those bridges so saved/replayed layouts drift-fail if
local generated enemy data changes. Placement seeds only the still-vanilla free
drops as assumed inventory, using `door_drop_total - active key pots - active
enemy key drops` per dungeon; the door oracle counts active pot, forced enemy,
and ordinary enemy check key sources only when their door regions and generated
source predicates are satisfied.
...
| 117→118 | **Dungeon enemy checks** — `enemy_drop_checks=dungeon` is exposed for Wild/Retro and Dungeon small-key seeds and adds ordinary dungeon `Enemy` locations from the local ROM-derived registry. | Existing `off`/`keys` rows stay unchanged. Three new dungeon-enemy corpus rows pin Wild, Dungeon, and door-shuffle behavior. |
...
| 162→164 | **Cave-interior region drift, and upstream pinning for the entries nothing checked** — `chest_shell_game` (the Kakariko Gamble Game door) declared `LightWorld_NorthWest` for a door ALTTPR declares in `LightWorld/South.php`. Same class as the 159 drift and hidden the same way: the entry has no locations, so `Entrance_SelfCheck` (2) never cross-validated it. Every location-less entry now carries `upstream_regions`, derived from the upstream PHP by door id and enforced by `gen_entrance_door_rows.py --check`. (163 was claimed by an unmerged branch.) | **All 246 corpus digests unchanged** — the two regions are reachability-equivalent on every corpus seed. The bump is taken under the inert-change exception's "when in doubt" clause: unlike the cases that exception lists, this one is inert only for the settings the corpus covers, and it does change hint text and tracker grouping for whatever lies behind that door. |

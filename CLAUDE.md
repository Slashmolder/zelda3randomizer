...
- **The Makefile has NO header-dependency tracking** (no `-MMD`/`.d`/`-include`). Editing a widely-included header (e.g. `kGeneratorVersion` in `rando.h`) does NOT recompile the `.c` TUs that include it — `make` rebuilds only TUs whose own `.c` changed (or whose gitignored codegen output you `rm`'d). A partial rebuild then ships stale objects carrying the OLD value, with a subtle symptom: e.g. one TU stamps a freshly-generated share-file with the new version while another checks it with the old → only the race-mode corpus entries fail (`--reveal-spoiler: FAILED (5): … different generator version`) while placement digests stay correct. **`make clean` after any header edit.** MSBuild/vcxproj DOES track header deps, so this is a Makefile/WSL-only trap.

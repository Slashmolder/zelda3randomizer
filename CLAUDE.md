...
- **Windows + TCC**: deprecated/unsupported. It cannot compile the recursive randomizer sources or C++ settings UI; use Visual Studio/MSBuild.
- **Nintendo Switch**: `cd src/platform/switch && make` (requires DevKitPro + `switch-sdl2`).

The Makefile builds the C core plus `src/rando/*.c` and the C++ Dear ImGui settings window. Randomizer validation is driven by `python assets/scripts/run_rando_validation.py quick|ci|full`; side-by-side ROM execution remains the strongest vanilla-equivalence check.
...
- **Keep generated/extracted data out of tracked files.** Asset data lives in **gitignored** artifacts (`zelda3_assets.dat`, `assets/dungeon/*.yaml`, `assets/rando/chest_table.gen.bin`), read at runtime/codegen — never inline large data tables into source. Structural facts (addresses in `variables.h`/`features.h`, the MIT ALTTPR `CHEST_NAME_BY_ROM_ADDR` catalog) are fine. `assets/scripts/check_no_embedded_data.py` (in `rando_ci.yaml`) fails the build on any long inline hex blob; allow a true exception with `embedded-data-guard: allow <reason>`.
  - **Trap — these gitignored assets can fail open at runtime.** If `assets/rando/chest_table.gen.bin` is absent when codegen runs, `src/rando/chest_lookup.h` is empty and chests resolve to their vanilla items. The grant self-test group and full artifact verification now guard the automated/full workflows, but an ad-hoc assetless build intentionally has zero local registries; check artifact counts first when placement and runtime grants disagree.
- **`-Werror` is on.** The CI matrix builds on Linux and macOS via `make -j$(nproc) zelda3`; treat warnings as build breaks.
- **GNU Make tracks headers with compiler-emitted `.d` files.** Do not work around dependency bugs with routine `make clean`; `check_make_depfiles.py` guards ordinary/generated-header rebuilds, no-op codegen, and clean wiring. Randomizer codegen intentionally remains always-run because optional gitignored inputs can disappear.
- **Use the validation profiles.** `run_rando_validation.py quick` is the edit loop; `full` performs artifact preparation plus a mandatory rebuild before every source/runtime/corpus/performance gate. Requested missing prerequisites fail closed.
- **The playable-slot path has its own automated matrix.** `check_rando_slot_path.py` compares `Rando_GenerateSlot` persistence and digest parity to the CLI path. Grant/event self-test groups cover runtime state transitions, but gameplay and rendering still require owner playtest; do not report automation as completing that gate.
...
**When mining structural data from a large source, GENERATE it from the source via a committed script — never hand-transcribe a derived summary.** A summary (even a careful sub-agent's) is lossy, and the loss is SILENT. The enemy-shuffle constraint tables were first built from an extraction agent's summary; that summary dropped Walking Zora's second required sheet (→ a "hybrid zora+buzzblob" garbage render), two slot-2 NPC ids from the pin set, and the 8 `AddGroup` village NPCs entirely — three separate playtest/audit-caught bugs, all the same root cause. The fix that ended the class: `assets/scripts/gen_enemy_shuffle_tables.py` parses the authoritative Enemizer `SpriteRequirement.cs` directly and emits the C tables, asserting its own invariants (disjointness, completeness). Prefer a committed generator + generated-but-checked-in source (structural facts are allowed inline, see the embedded-data guard) over a transcription you can't re-derive. The generator IS the spec for that data.
...
Vanilla ALTTP repeatedly uses one item/state as a stand-in for "the player reached milestone Y," because in vanilla `has(X)` and `past(Y)` are equivalent. Rando breaks that equivalence by shuffling the proxy. This is the single most common bug class in the randomizer work. The named grant self-test and transaction-source guard now cover placement resolution and representative delivery/event state transitions, but the placement corpus does not execute runtime code and gameplay/consume proxies remain only partially modelled. **End-to-end playtest remains the final net.** When reviewing a rando change, audit these forms:
...
- **Draw-side resolution that must mirror the grant chain** — a feature that *renders* a placed item (e.g. field-item sprites at heart-piece/book/etc. locations) must consume the same side-effect-free `RandoGrantPlan` used by delivery. The generated semantic metadata classifies every registry item and the grant self-test exhaustively checks plan/effect/display agreement, progressive boundaries, shared-byte preservation, and lossless rejection. Never rebuild a parallel `progressive_to_lttp` / direct-icon decision tree at a draw site. (Sibling gotcha for sprite draws: the per-sprite OAM budget `((sprite_flags2&0x1f)+1)*4` is in BYTES — a standing sprite reserves ONE entry, so a 2-tile (8×16) draw needs an explicit `Oam_AllocateFromRegion*` or the 2nd tile is scene-dependently clobbered.)

- **Grant-site fallback that silently drops the placed item** — a minigame/NPC path that accepts only some placement classes and falls back to its vanilla consolation prize loses every other placement class. The Digging Game once shipped exactly that bug. Every randomized site now must use the public prepare/commit transaction API, branch explicitly on `NotActive`, `Accepted`, `AlreadyChecked`, and `Blocked`, and postpone irreversible caller state until accepted delivery. `check_grant_consumers.py` rejects new raw resolver/delivery bypasses, while `--rando-grant-check` exercises the semantic matrix and representative handler retry/cleanup paths twice in one process. When adding a site, still audit its vanilla fallback and replay choreography; source guards cannot prove frame-level control flow.

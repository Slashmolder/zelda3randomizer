...
// an F12 dump). 0x665 hook-calls, 0x666 cumulative slots-changed, 0x667 last
// room's blocked-slot bitmask. Flip to 0 before merge.
#define ES_RESHUFFLE_DIAG 1

// We reshuffle subgroup slots 0,1,2 (the enemy slots; slot 3 is mostly objects).
#define ES_RESHUFFLE_SLOTS 3
...
static const uint8 *const kEsDungeonPool[ES_RESHUFFLE_SLOTS]   = { kEsDungeonPool0, kEsDungeonPool1, kEsDungeonPool2 };
static const uint8 *const kEsOverworldPool[ES_RESHUFFLE_SLOTS] = { kEsOverworldPool0, kEsOverworldPool1, kEsOverworldPool2 };
static const uint8 kEsDungeonPoolCount[ES_RESHUFFLE_SLOTS]   = { 3, 2, 7 };
static const uint8 kEsOverworldPoolCount[ES_RESHUFFLE_SLOTS] = { 3, 2, 9 };
...
static const uint8 kSheetNeed[256] = {
  ...
  [0x24]=0x81, [0x25]=0x81, [0x26]=0x82, [0x27]=0x88, [0x28]=0x80, [0x29]=0x81, [0x2A]=0x80, [0x2B]=0x80,
  [0x2C]=0x84, [0x2D]=0x80, [0x2E]=0x80, [0x2F]=0x80, [0x30]=0x80, [0x31]=0x81, [0x32]=0x81, [0x33]=0x80,
  [0x34]=0x80, [0x35]=0x80, [0x36]=0x84, [0x37]=0x80, [0x38]=0x80, [0x39]=0x88, [0x3A]=0x88, [0x3B]=0x80,
  [0x3C]=0x80, [0x3D]=0x80, [0x3E]=0x88, [0x3F]=0x80, [0x40]=0x88, [0x41]=0x82, [0x42]=0x82, [0x43]=0x82,
  ...
  [0x6C]=0x80, [0x6D]=0x84, [0x6E]=0x84, [0x6F]=0x84, [0x71]=0x81, [0x72]=0x88, [0x73]=0x81, [0x74]=0x80,
  [0x75]=0x80, [0x76]=0x80, [0x78]=0x87, [0x7A]=0x8F, [0x7B]=0x80, [0x7C]=0x81, [0x7D]=0x88, [0x7E]=0x81,
...
void EnemyShuffle_ReshuffleCurrentRoomSheets(const uint8 *tileset_row) {
  ...
  if (g_ram[0x665] < 0xff) g_ram[0x665]++;                          // hook calls
  if (g_ram[0x666] <= (uint8)(0xff - changed)) g_ram[0x666] += changed; // cumulative slots changed
  g_ram[0x667] = blocked;                                           // last room's blocked mask
#endif
}

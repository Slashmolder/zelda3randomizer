...
// ---------------------------------------------------------------------------
#define kGeneratorVersion 47u  // 46→47: fork-extension hint NPCs (Storyteller + Kakariko/Dark-World Fortune Tellers, ids 17-19) add three entries to the spoiler `hints[]` for any hints-on seed. The race-mode anti-tamper stamp is a SHA-256 over the full spoiler JSON, so this changes the stamp bytes for race-mode + hints-on seeds. Without this bump a v46 race seed (minted before the fork NPCs) would regenerate a 3-entry-larger hints[] at reveal → false `kRandoReveal_StampMismatch` ("tampered"). The bump makes the version gate fire first → honest `kRandoReveal_VersionMismatch` ("regenerate the seed"). Placement/sphere digests are unchanged (hints are generated post-placement; generator_version is not an RNG input), so the corpus is byte-identical — verified, not regenerated.

...
// ---------------------------------------------------------------------------
#define kGeneratorVersion 97u  // 96->97: add-rando-pot-sanity (owner F12 Swamp pot audit) - room 0x036 has two top-right pots reachable after the Swamp key+Hammer gate, but the other six pots require Hookshot; the committed pot table had the whole room on the top-right gate. Split the room in pot_logic_overrides.yaml/pots.gen.yaml and add ROM-free override-sync CI so this class cannot silently drift again. Placement-affecting only for active pot seeds whose fill sees the corrected Swamp predicates; corpus observed 2 Inverted/Retro cave+pot guard entries move, rest byte-identical. - 95→96: add-rando-pot-sanity (owner-playtest audit, DUNGEON-room class) — fix 2 in-dungeon pot-room region mislabels the grid-adjacency flood mis-bound across a dungeon boundary / via supertile (non-door) adjacency; floor-bit-aware door-graph (src/rando/door_tables.gen.c) + key-pot data confirmed. 0x04b (2 PoD Jelly-Hall/Mimics LOOT pots, Arrow10+HeartRefill) was wrongly GanonsTower — the PoD|GT engine-room boundary sits at 0x4b|0x4c so the flood leaked across — → PalaceOfDarkness, gated `(CanBombThings() OR HAS_ITEM(Boots))` (reached lobby→warp-room→warp-hint by the KEY-FREE route, door_tables dungeon=5; no key door on any path). 0x096 (8 GT Staredown/Torch-Cross LOOT/EMPTY pots) was wrongly TowerOfHera — the 097→087 one-way drop is supertile-grid adjacency, NOT dungeon membership — → GanonsTower_Lobby, gate `BigKey_TowerOfHera`→`BigKey_GanonsTower` (room is door-graph-proven unreachable without the GT big key; the small-key economy is layered SEPARATELY by the POT_KEYS wrap from pot_key_depth.gen.yaml's door-rando GT depths, so the base gate is the big key only). Both rooms hold LOOT/EMPTY pots only (no key pots) → key economy + Keys-tier/Off/non-pot seeds byte-identical; Contents/All pot seeds move (re-regioned/re-gated). 5 sibling flood-flagged rooms (0x031/0x054/0x056/0x05b/0x064) re-verified CORRECT and left as-is (floor-bit-confounded false positives — e.g. engine 0x031 = door-rando 0x71 = HC sewers, genuinely HCE not ToH). — 94→95: add-rando-pot-sanity (owner-playtest audit) — fix REGION MISLABELS for shared overworld cave/house pot rooms that the grid-adjacency region-flood (and one stale reviewed override) bound to the wrong region, making them falsely sphere-0. Owner F12 + fork-location cross-check: 0x114 Pond-of-Wishing→DarkWorld_Mire; 0x11a storyteller-cave→DarkWorld_NorthEast (= the PoD entrance region); 0x119 Blind's-Hideout→LightWorld_NorthWest (was wrongly DarkWorld_NorthWest); 0x11f Lumberjack's-House→LightWorld_NorthWest (was wrongly DarkWorld_NorthWest); 0x10c Mimic-Cave gains the fork's Hammer+Mirror+TR-keys gate (was TRUE); 0x11b refill_cave_1 SPLIT per-entrance via a new gen_pot_tables.py `pot_room_split` mechanism (8 pots Graveyard-Ledge gate / 6 pots Cave-45 gate). ALSO made the task-#25 Desert Palace mislabel fix (rooms 0x43/0x53→DesertPalace_Lobby) generator-REPRODUCIBLE: it was a hand-edit in the committed pots.gen.yaml that any regen reverted (the flood binds 0x43→ThievesTown, the old override bound 0x53→HCE), silently breaking the DP key economy; moved into pot_logic_overrides.yaml. Pots-off / non-pot seeds byte-identical; pot seeds move (the re-gated cave pots). — 93→94: add-rando-pot-sanity (audit, sibling class) — two more RAW-settings-vs-normalized divergences fixed via the Effective-accessor idiom. (1) The placer acceptance gate (Accessibility_SeedAcceptable / accessibility_reachability_ok), the spoiler, and the CLI error path read RAW accessibility; apply_derived_rules forces Locations under goal==Completionist only on the private hash copy, so `goal=completionist,accessibility=none` (explicit key after goal) skipped the 100%-locations sphere walk the share string promised — accept-bad-seed + non-reproducible. Now all read Settings_EffectiveAccessibility (Completionist→Locations). (2) Settings_PotShuffleForcedOff read RAW shuffle_cave_entrances with no world-state guard, so Inverted/Retro + a retained cave bit + pots forced pots OFF in the placer while the canonical hash (cave zeroed first) kept pots ON; now reads Settings_EffectiveShuffleCaveEntrances (cave honored only on Open/Standard, mirroring apply_derived_rules), so Inverted/Retro+cave+pots correctly generate WITH pots. Corpus byte-identical for every existing seed (bare completionist already had accessibility=Locations; no Inverted/Retro+cave seed existed); new completionist+accessibility=none and inverted+cave+pot coverage seeds added. — 92→93: add-rando-pot-sanity (audit fix) — route ALL pot-key gates (eval_pot_keys_on/wild/dungeon in rando_logic.c, pot_keys_dungeon_active in rando_placement.c) through the new shared Settings_PotKeysActive helper so they honor Settings_PotShuffleForcedOff (door OR CAVE-entrance shuffle), matching pot_active/BuildItemPool. Root cause: the placer consumes RAW settings (Settings_CanonicalSerialize runs apply_derived_rules only on a private copy for the hash), so the door-only pot-key predicates fired under cave-entrance shuffle and gated a pot-bearing dungeon's deep locations on pot keys that pot_active never pooled → cave-entrance + pot + wild/dungeon-keys seeds were wrongly REFUSED (reproduced by audit). Corpus byte-identical for every existing seed (the door-only gate already matched for non-cave seeds; the cave guard seed uses Vanilla keys, which the wild/dungeon ops never trigger); new cave+pot+dungeon-keys / +wild-keys coverage seeds added. — 91→92: add-rando-pot-sanity (audit fix) — normalize pot_shuffle OFF under CAVE-entrance shuffle (Settings_PotShuffleForcedOff), not just door shuffle. Cave/house pot location IDs sit above the per-location entrance region-override range (Entrance_ApplyRegionOverrides remaps only the caves' <512 chest IDs), so a cave/house pot evaluated from its VANILLA overworld region while the runtime reaches the interior via the shuffled entrance — certifying progression in a pot the layout doesn't make reachable. Now the placer, settings_hash, runtime (pot_active), and spoiler all consult the same predicate. Dungeon-entrance shuffle is edge-based and unaffected. Corpus byte-identical (no cave-entrance+pot seed existed); a c-entrance-caves-pot-forced-off guard seed added. — 90→91: add-rando-pot-sanity (task #25) — the within-dungeon pot-key SHUFFLE replaces the pinning. Under DUNGEON small keys a dungeon's POT keys are now first-class shuffled CHECKS (location_is_prepinned no longer pins LOCTYPE_Pot under Dungeon; BuildItemPool pools them under !=Vanilla). Gated by the in-context graduated requirement = the prover SHORTEST-PATH (min-depth, --dump-key-depth `mindepth=`), via the new OP_POT_KEYS_DUNGEON: chest/boss/prize gate their LOC min-depth, KEY pots their EXACT region min-depth (door-rando key_drop_data DROP-region join), loot/empty pots the room-MAX min-depth — never the worst-case (circular for keys). The nonpot enemy/guard drops are free-granted into the assumed inventory (kPotNonpotDropCounts: EP+1/IP+3/MM+1/GT+1, selfchecked) so the gates stay satisfiable, mirroring pots-off "drops free". ALSO fixes a pre-existing data bug: 2 mislabeled Desert Palace pots (room 0x53 Beamos Hall→was HCE, 0x43 Desert Tiles 2→was TT; + their 10 room-mates) rebound to DesertPalace — this also corrects the shipped WILD economy (DP/HC/TT caps) and closes a chest-less-room TRUE() leak the 21-pass missed under the wrong labels. Corpus: the dungeon-keys pot seed + the wild-pot seed move (DP/HC/TT); pots-off / vanilla / non-pot byte-identical. — 89→90: add-rando-pot-sanity (task #25) — DUNGEON-keys half (pinning, now superseded). Under dungeon small keys, a dungeon's POT keys now stay VANILLA (location_is_prepinned pins each LOCTYPE_Pot small-key location to its own key, dropped in place exactly like pots-off) while the CHEST keys still shuffle in-dungeon. Previously the key pots were open checks but their keys were never pooled (kVanillaSmallKeyCounts is the chest count) → they vanished → the dungeon went short keys → runtime strand (invisible to --generate-seed, a consistent under-gate). Pinning is provably safe (vanilla key-pot economy), validated: dungeon pot_shuffle keys/all stay 100%-accessible, all 17 key pots pin to their own SmallKey, 0 unreachable. The full within-dungeon pot-key SHUFFLE (in-context graduated key logic, no clean automated source for vanilla doors) remains a follow-on. WILD keys (the prior half) still shuffle pot keys into the general pool. Corpus: ONLY pot-all-dungeon-keys-open moves (the lone dungeon-keys pot seed); vanilla/wild/pots-off all byte-identical (128/129). New dungeon+pot corpus entry added. — 88→89: add-rando-pot-sanity (task #25) — model dungeon POT keys under WILD keys. When pot_shuffle turns a dungeon's key pots into live checks, the pot small keys must (a) enter the item pool, or they vanish (kVanillaSmallKeyCounts is the placed/chest count only) — the wild/pot strand the owner hit, and (b) gate the dungeon's deep locations + the pots themselves, or the placer mis-reaches them. Both gate on the new OP_POT_KEYS_WILD (pot_shuffle>=Keys AND door-off AND small keys WILD): rando_placement.BuildItemPool pools each active key pot's vanilla SmallKey (GenericKey under Retro), and rando_logic_gen wraps the 54 affected chest/boss/prize locations + 102 pot rooms as `<vanilla> AND (NOT POT_KEYS_WILD OR HAS_AMOUNT(SmallKey_X, full))`, where `full` is the prover WORST-CASE key-door depth (--dump-key-depth) CAPPED at the pooled key count (chest+pot_keys) — the external requirement under wild, since nonpot drops auto-collect in context. DUNGEON-keys in-context modeling is a follow-on (kept on the vanilla branch). Validated 100%-accessible (accessibility=items) across wild pot_shuffle keys/contents/all + Retro + triforce-hunt. Corpus: ONLY pot-all-retro-ganon moves (the lone wild+pot seed); every pots-off / Vanilla-keys-pot / dungeon-keys-pot / non-pot seed stays byte-identical (126/127). New wild+pot corpus entries added. — 87→88: add-rando-pot-sanity — close the in-dungeon chest-less-pot sphere-0 LEAK class the kGen-87 guard surfaced (21 flagged rooms). 10 get a reviewed room_can_reach ENTER gate, traced from the door-shuffle compiled door tables (src/rando/door_tables.gen.c — REAL per-door SmallKey/BigKey flags + region graph, authoritative over the grid-adjacency pot_dump whose listed edges are often walls; cross-checked vs ALTTPR PHP chest predicates; never-under-gate): TR 015/023=SmallKey_TurtleRock>=3, PoD 01b=CanShootArrowsL1, SW 049=SmallKey_SkullWoods, TT 054=BigKey_ThievesTown, DP 063=CanLiftRocks (free back-door floor, pre-key), GT 05d/06c/06d=CanShootArrowsL1 AND SmallKey_GanonsTower>=1, GT 09b=Hammer AND Hookshot AND SmallKey_GanonsTower>=1. The 3 key pots are deadlock-safe (063 pre-key on the free floor; 09b non-blocking; 09f free). The other 11 are GENUINELY FREE (entrance/lobby, GT Hope/catwalk, MM bridge, TR hub, + 4 IP free-fall descent rooms incl. the 09f key pot — key-free, Iced T Room=TRUE) and go in pot_logic_overrides.yaml room_free:, which the now-HARD-FAILING guard skips. 4 corpus entries move — the Contents/All-tier pot seeds (incl. All+open-dungeon-keys, where the now-gated key pots 063/09b shift the fill); the Keys-tier (pinned), Off, door, and every non-pot seed stay byte-identical. Corpus regenerated.

...
// ---------------------------------------------------------------------------
#define kGeneratorVersion 62u  // 61→62: add-rando-enemy-shuffle §7.4 — the sprite-group SHEET reshuffle. When enemy_shuffle is on, the GFX-sheet load (Gfx_LoadSpritesInner / InitializeTilesets) may re-assign subgroup slot 2 per (seed, room/area) so a wider, cross-family enemy pool spawns. This is a NEW LIVE RUNTIME behavior of the EXISTING enemy_shuffle axis (owner decision: always-on when enemy_shuffle, NO new canonical axis) — so kSettingsCanonicalLen, the canonical layout, and default-settings settings_hash are ALL unchanged. Like the base axis it is ORTHOGONAL to item placement (draws no fill RNG, adds no logic predicate): placement_digest_hex stays BYTE-IDENTICAL for every seed. The bump version-locks the changed runtime behavior so a shared race seed (enemy_shuffle=on) reproduces the same enemy layout across binaries and an older binary surfaces the upgrade warning. Corpus regenerates ONLY its manifest generator_version; EVERY digest is byte-identical (no corpus seed enables enemy_shuffle). Corpus MUST be regenerated (bump_rando_corpus.py --apply).
                               // 60→61: add-rando-enemy-shuffle adds the enemy (sprite-type) substitution axis as a LIVE runtime slot axis. enemy_shuffle bit-packs into the previously-zero canonical pad byte [26] bit0 (kSettingsCanonicalLen stays 28 — no size-coupling cascade). Enemy shuffle is ORTHOGONAL to item placement: it draws no fill RNG and adds no logic predicate (the fresh-eyes sweep confirmed every CanKill<X> kill macro is a player-firepower test over bosses/mini-bosses, all EXCLUDED from the substitution pool), so placement_digest_hex is BYTE-IDENTICAL for every seed (incl. enemy_shuffle=on) and default-settings settings_hash is byte-identical (pad bit 0 by default). The bump version-locks the new LIVE runtime axis (an older binary surfaces the upgrade warning rather than silently ignoring an enemy-shuffle slot) and reflects the per-seed settings_hash change for an enemy_shuffle=on seed (its [26] byte flips to 1). Corpus regenerates ONLY its manifest generator_version; EVERY digest is byte-identical (no corpus seed enables the axis). Corpus MUST be regenerated (bump_rando_corpus.py --apply). NOTE: task brief said 58→59; the worktree base already carried Phase D bumps to 60, so this lands as 60→61.

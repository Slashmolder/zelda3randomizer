...
def load_pots(path: Path, logic_regions: dict[str, RegionDef] | None = None):
    ...
    if not depth_tbl.exists():
        sys.exit(f"{path} is present but {depth_tbl} is missing. Run "
                 "assets/scripts/gen_pot_key_depth.py after --dump-key-depth so "
                 "door+pot bridge rows and pot-key gates are generated.")
    doc = load_yaml(path)
